...
class AddressDao @Inject()(implicit ec: DatabaseExecutionContext) {
  def createAddress(addressRequest: AddressRequest)(implicit connection: Connection): Address = SQL(
    s"""
       | INSERT INTO address (
       |    a_line_1,
       |    a_line_2,
    ...
    ).executeInsert(scalar[Long].single) match {
    case id: Long => Address(id, addressRequest)
  }
}
